...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    private func getOutlineViewItems(for item: CEWorkspaceFile) -> [CEWorkspaceFile] {
        if let cachedChildren = filteredContentChildren[item] {
            return cachedChildren
                .sorted { lhs, rhs in
                    host?.sortFoldersOnTop == true ? lhs.isFolder && !rhs.isFolder : lhs.name < rhs.name
                }
        }

        if let host, let children = host.fileManager?.childrenOfFile(item) {
            if !host.navigatorFilter.isEmpty {
                let filteredChildren = children.filter {
                    fileSearchMatches(host.navigatorFilter, for: $0)
                }
            ...
            return children
                .sorted { lhs, rhs in
                    host.sortFoldersOnTop ? lhs.isFolder && !rhs.isFolder : lhs.name < rhs.name
                }
        }
    ...
    func outlineView(
        ...
        for fileItemURL in fileItemURLS {
            ...
            var srcFileItem: CEWorkspaceFile? = host?.fileManager?.getFile(fileItemURL.path)
